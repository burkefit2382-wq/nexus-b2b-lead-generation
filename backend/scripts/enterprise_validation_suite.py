...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-p95-ms", type=float, default=500.0, help="Maximum per-endpoint p95 latency (ms).")
...
async def run_availability_check(
    ...
    for _ in range(probes):
        ...
        except Exception as exc:  # noqa: BLE001 - surface request failures in output without crashing CI.
            latencies.append((time.perf_counter() - start) * 1000.0)
            failures.append(str(exc))
    ...
    with temp_env(
        {
            ...
            "HUBSPOT_PRIVATE_APP_TOKEN": "pat-do-not-leak-token",
        }
    ):
        response = await client.get("/api/config-status")
        body = response.text
        leaked = any(token in body for token in (redaction_secret, "pat-do-not-leak-token"))
        checks.append(
            {
    ...
    with temp_env(
        {
            ...
            "PRICE_ID": "price_test",
            "HUBSPOT_PRIVATE_APP_TOKEN": "pat_test",
            "HEALTHCHECK_SKIP_NETWORK_PROBES": "true",
        }
    ):
