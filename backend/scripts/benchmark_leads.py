...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--nexus", required=True, help="Path to your Nexus CSV or JSON")
...
def domain_overlap(rows_a: list[dict[str, str]], rows_b: list[dict[str, str]]) -> int:
    domains_a = {email_domain(find_value(row, "email")) for row in rows_a if email_domain(find_value(row, "email"))}
    domains_b = {email_domain(find_value(row, "email")) for row in rows_b if email_domain(find_value(row, "email"))}
